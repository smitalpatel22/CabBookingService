...
public class RidesManagerService {
    ...
    public CabIdleCheckResponse cabIdleStats(@NonNull final Integer cabId, Date startDate, Date endDate) {
        Cab cab = cabsManagerService.getCab(cabId);
        List<Trip> trips = tripRepository.findAllByCabAndBookingDateBetween(cab, startDate, endDate);

        long rangeSupplied = endDate.getTime() - startDate.getTime();
        log.info("Total Range Supplied: {}", rangeSupplied);

        long idleTime = rangeSupplied - 3000*trips.size();
        return CabIdleCheckResponse.builder().idleTime(idleTime).build();
    }
